        ...
        {calendarData.map((day, index) => {
          ...
          return (
            <DayComponent
              ...
              className={`h-36 border border-gray-200 rounded-lg p-2 text-left transition-all duration-200 ${day.isWeekend ? 'bg-gray-100' : 'bg-white'} ${isClickable ? 'cursor-pointer hover:bg-gray-50 hover:shadow-md' : ''}`}
            >
              <p className="text-base font-bold text-right">{day.date}</p>
              <div className="mt-1.5 space-y-1">
                {day.leaves.slice(0, 4).map((leave, leaveIndex) => (
                 ...
                 {day.leaves.length > 4 && (
                   <p className="text-xs font-bold text-nimasa-blue mt-1">+ {day.leaves.length - 4} more</p>
                )}
              </div>
            </DayComponent>
          );
