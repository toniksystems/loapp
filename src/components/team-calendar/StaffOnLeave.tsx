...
const StaffOnLeave = () => {
    return (
        <div className="bg-white rounded-3xl shadow-md p-6 border border-gray-200">
            <h3 className="text-2xl font-medium text-nimasa-dark-text mb-6">Staff on Leave This Month</h3>
            <div className="space-y-4">
                {staffOnLeave.map((staff, index) => (
                    <div key={index} className="flex items-center gap-4">
                        <img src={staff.avatarUrl} alt={staff.name} className="h-14 w-14 rounded-full" />
                        <div>
                            <p className="text-lg font-medium text-nimasa-dark-text">{staff.name}</p>
                            <div className="flex items-center gap-2">
                                <div className={`w-3 h-3 rounded-full ${staff.leaveColor}`}></div>
                                <p className="text-base text-gray-600">{staff.leaveType}</p>
                            </div>
                        </div>
                    </div>
                ))}
