      ...
      >
        <div className="flex items-start justify-between py-6 px-8 border-b border-gray-200">
          <div className="flex flex-col items-center w-full">
            <img src={logoUrl} alt="NIMASA Logo" className="h-24 w-24 mb-3" />
        ...
        <nav className="flex-grow pt-5 overflow-y-auto" onClick={() => setIsOpen(false)}>
          <ul className="space-y-3">
            {adminNavItems.map((item, index) => (
              <li key={`admin-${index}`} className="px-6">
                <NavLink
                  to={item.path}
                  className={({ isActive }) =>
                    `flex items-center gap-5 px-5 py-3 rounded-xl transition-colors duration-200 whitespace-pre-line text-nimasa-dark-text hover:bg-nimasa-blue/10 ${
             ...
             <ul className="space-y-2">
                {utilityItems.map((item, index) => (
                  <li key={`util-${index}`} className="px-6">
                    <NavLink
                      to={item.path}
                      className="flex items-center gap-5 px-5 py-3 rounded-xl transition-colors duration-200 whitespace-pre-line text-nimasa-red"
                    >
                      <item.icon className="h-7 w-7 text-nimasa-red" />
                      <span className="text-xl font-medium">{item.name}</span>
                    </NavLink>
                  </li>
                ))}
