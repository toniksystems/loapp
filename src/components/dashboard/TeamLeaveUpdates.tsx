...
const TeamLeaveUpdates = () => {
    return (
        <div className="bg-white rounded-2xl shadow-lg p-8">
            <h2 className="text-2xl font-medium text-nimasa-dark-text mb-8">Team Leave Updates</h2>
            <div className="space-y-8">
                {updates.map((update, index) => (
                    <div key={index} className="flex items-start gap-6 border-b border-gray-100 pb-7 last:border-b-0 last:pb-0">
                        <img src={update.avatarUrl} alt="avatar" className="h-14 w-14 rounded-full" />
                        <div className="flex-grow">
                            <p className="font-medium text-xl text-nimasa-dark-text">{update.message}</p>
                            <p className="text-lg text-gray-600">{update.details}</p>
                        </div>
                        <span className="text-lg text-gray-500 flex-shrink-0">{update.time}</span>
                    </div>
                ))}
            </div>
        </div>
