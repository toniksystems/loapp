...
const TeamLeaveUpdates = () => {
    return (
        <div className="bg-white rounded-2xl shadow-lg p-5">
            <h2 className="text-lg font-medium text-nimasa-dark-text mb-5">Team Leave Updates</h2>
            <div className="space-y-5">
                {updates.map((update, index) => (
                    <div key={index} className="flex items-start gap-3 border-b border-gray-100 pb-5 last:border-b-0 last:pb-0">
                        <img src={update.avatarUrl} alt="avatar" className="h-10 w-10 rounded-full" />
                        <div className="flex-grow">
                            <p className="font-medium text-base text-nimasa-dark-text">{update.message}</p>
