...
const TeamLeaveUpdates = () => {
    return (
        <div className="bg-white rounded-2xl shadow-md p-4">
            <h2 className="text-base font-medium text-nimasa-dark-text mb-4">Team Leave Updates</h2>
            <div className="space-y-4">
                {updates.map((update, index) => (
                    <div key={index} className="flex items-start gap-3 border-b border-gray-100 pb-3 last:border-b-0 last:pb-0">
                        <img src={update.avatarUrl} alt="avatar" className="h-8 w-8 rounded-full" />
                        <div className="flex-grow">
                            <p className="font-medium text-sm text-nimasa-dark-text">{update.message}</p>
                            <p className="text-xs text-gray-600">{update.details}</p>
                        </div>
                        <span className="text-xs text-gray-500 flex-shrink-0">{update.time}</span>
                    </div>
                ))}
            </div>
        </div>
