...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      ></div>
      <aside
        className={`w-72 bg-white h-screen flex flex-col fixed shadow-2xl z-40 transform transition-transform lg:transform-none ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-start justify-between py-4 px-5 border-b border-gray-200">
          <div className="flex flex-col items-center w-full">
            <img src={logoUrl} alt="NIMASA Logo" className="h-16 w-16 mb-2" />
            <h2 className="text-sm font-medium text-nimasa-dark-text">NIMASA Leave Portal</h2>
          </div>
          <button onClick={() => setIsOpen(false)} className="lg:hidden text-gray-500 hover:text-nimasa-red">
            <X size={24} />
          </button>
        </div>
        <nav className="flex-grow pt-3 overflow-y-auto" onClick={() => setIsOpen(false)}>
          <p className="px-6 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Employee</p>
          <ul className="space-y-1">
            {navItems.map((item, index) => (
              <li key={`emp-${index}`} className="px-3">
                <NavLink
                  to={item.path}
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2 rounded-lg transition-colors duration-200 whitespace-pre-line text-nimasa-dark-text hover:bg-nimasa-blue/10 ${
          ...
          <p className="px-6 mt-4 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Manager</p>
           <ul className="space-y-1">
            {managerNavItems.map((item, index) => (
              <li key={`mgr-${index}`} className="px-3">
                <NavLink
                  to={item.path}
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2 rounded-lg transition-colors duration-200 whitespace-pre-line text-nimasa-dark-text hover:bg-nimasa-blue/10 ${
        ...
        <div className="py-3 border-t border-gray-200">
             <ul className="space-y-0.5">
                {utilityItems.map((item, index) => (
                  <li key={`util-${index}`} className="px-3">
                    <NavLink
                      to={item.path}
                      className="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors duration-200 whitespace-pre-line text-nimasa-red"
                    >
                      <item.icon className="h-5 w-5 text-nimasa-red" />
                      <span className="text-sm font-medium">{item.name}</span>
                    </NavLink>
                  </li>
                ))}
