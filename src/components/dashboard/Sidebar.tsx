  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-grow pt-8" onClick={() => setIsOpen(false)}>
            ...
            {navItems.map((item, index) => (
              <li key={index} className="px-2">
                <NavLink
                  to={item.path}
                  className={({ isActive }) =>
                    `flex items-center gap-4 px-6 py-3 rounded-lg transition-colors duration-200 whitespace-pre-line ${
                      item.isLogout
                        ? 'text-nimasa-red'
                        : 'text-nimasa-dark-text hover:bg-nimasa-blue/10'
                    } ${
                      isActive && !item.isLogout ? 'bg-nimasa-blue/10 text-nimasa-blue font-bold' : ''
                    }`
                  }
                >
                  <item.icon className={`h-6 w-6 ${item.isLogout ? 'text-nimasa-red' : ''}`} />
