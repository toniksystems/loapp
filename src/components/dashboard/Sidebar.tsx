...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      ></div>
      <aside
        className={`w-96 bg-white h-screen flex flex-col fixed shadow-2xl z-40 transform transition-transform lg:transform-none ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-start justify-between py-6 px-8 border-b border-gray-200">
          <div className="flex flex-col items-center w-full">
            <img src={logoUrl} alt="NIMASA Logo" className="h-28 w-28 mb-3" />
        ...
        <nav className="flex-grow pt-6 overflow-y-auto" onClick={() => setIsOpen(false)}>
          <p className="px-9 text-lg font-semibold text-gray-400 uppercase tracking-wider mb-5">Employee</p>
          <ul className="space-y-3">
            {navItems.map((item, index) => (
              <li key={`emp-${index}`} className="px-6">
                <NavLink
                  to={item.path}
                  className={({ isActive }) =>
                    `flex items-center gap-6 px-6 py-3.5 rounded-xl transition-colors duration-200 whitespace-pre-line text-nimasa-dark-text hover:bg-nimasa-blue/10 ${
          ...
          <p className="px-9 mt-9 text-lg font-semibold text-gray-400 uppercase tracking-wider mb-5">Manager</p>
           <ul className="space-y-3">
            {managerNavItems.map((item, index) => (
              <li key={`mgr-${index}`} className="px-6">
                <NavLink
                  to={item.path}
                  className={({ isActive }) =>
                    `flex items-center gap-6 px-6 py-3.5 rounded-xl transition-colors duration-200 whitespace-pre-line text-nimasa-dark-text hover:bg-nimasa-blue/10 ${
        ...
        <div className="py-6 border-t border-gray-200">
             <ul className="space-y-2">
                {utilityItems.map((item, index) => (
                  <li key={`util-${index}`} className="px-6">
                    <NavLink
                      to={item.path}
                      className="flex items-center gap-6 px-6 py-3.5 rounded-xl transition-colors duration-200 whitespace-pre-line text-nimasa-red"
                    >
                      <item.icon className="h-8 w-8 text-nimasa-red" />
                      <span className="text-2xl font-medium">{item.name}</span>
                    </NavLink>
                  </li>
                ))}
