...
const ApplicationStatusBadge: React.FC<StatusBadgeProps> = ({ status }) => {
  if (status === 'N/A') {
    return <span className="text-base text-gray-600 px-4">{status}</span>;
  }
  ...
  return (
    <div className={`flex items-center justify-center gap-1.5 py-2 px-4 rounded-full text-white text-base font-medium ${bgColor} w-32`}>
      {icon}
      <span>{text}</span>
    </div>
  );
