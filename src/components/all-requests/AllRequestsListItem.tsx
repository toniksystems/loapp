  ...
  return (
    <div className="grid grid-cols-12 gap-4 items-center py-3 px-4">
      ...
      <div className="col-span-2">
        ...
        <p className="text-sm text-gray-600 mt-1 ml-5">{leave.duration}</p>
      </div>

      {/* Status and Details */}
      <div className="col-span-3">
        <AllStatusBadge status={status} />
        <p className="text-xs text-gray-500 mt-1">{statusDetail}</p>
      </div>

      {/* Requested Date/Days */}
      <div className="col-span-2">
        <p className="font-medium text-sm text-nimasa-dark-text">{requested.date}</p>
        <p className="text-sm text-gray-600">{requested.days}</p>
      </div>

      {/* Actioned By/Date */}
      <div className="col-span-2">
        <p className="font-medium text-sm text-nimasa-dark-text">{actioned.date}</p>
        <p className="text-sm text-gray-600">{actioned.by}</p>
      </div>
    </div>
  );
};
