...
const PendingApprovals = () => {
    return (
        <div className="bg-white rounded-2xl shadow-md p-4 h-full">
            <h2 className="text-base font-medium text-nimasa-dark-text mb-4">Pending Approvals</h2>
            <div className="space-y-3">
                {pendingRequests.map((req, index) => (
                    <div key={index} className="bg-input-bg rounded-xl p-2.5 flex flex-col sm:flex-row items-center justify-between gap-2.5">
                        <div className="flex items-center gap-2.5">
                            <img src={req.avatar} alt={req.name} className="h-9 w-9 rounded-full" />
                            <div>
                                <p className="font-bold text-xs text-nimasa-dark-text">{req.name}</p>
                                <p className="text-[11px] text-gray-600">{req.leaveType}</p>
                                <p className="text-[11px] text-gray-600">{req.dates} ({req.days} days)</p>
                            </div>
                        </div>
                        <button className="bg-nimasa-blue text-white font-bold py-1 px-3 rounded-full text-xs shadow-md hover:bg-blue-700 transition-colors w-full sm:w-auto">
                            Review Request
                        </button>
                    </div>
                ))}
