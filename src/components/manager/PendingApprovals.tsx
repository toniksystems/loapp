...
const PendingApprovals = () => {
    return (
        <div className="bg-white rounded-2xl shadow-md p-6 h-full">
            <h2 className="text-xl font-medium text-nimasa-dark-text mb-6">Pending Approvals</h2>
            <div className="space-y-5">
                {pendingRequests.map((req, index) => (
                    <div key={index} className="bg-input-bg rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            <img src={req.avatar} alt={req.name} className="h-12 w-12 rounded-full" />
                            <div>
                                <p className="font-bold text-base text-nimasa-dark-text">{req.name}</p>
                                <p className="text-sm text-gray-600">{req.leaveType}</p>
                                <p className="text-sm text-gray-600">{req.dates} ({req.days} days)</p>
                            </div>
                        </div>
                        <button className="bg-nimasa-blue text-white font-bold py-2 px-5 rounded-full text-base shadow-md hover:bg-blue-700 transition-colors w-full sm:w-auto">
                            Review Request
                        </button>
                    </div>
                ))}
