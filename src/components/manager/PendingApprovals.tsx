...
const PendingApprovals = () => {
    return (
        <div className="bg-white rounded-2xl shadow-md p-4 h-full">
            <h2 className="text-lg font-medium text-nimasa-dark-text mb-5">Pending Approvals</h2>
            <div className="space-y-4">
                {pendingRequests.map((req, index) => (
                    <div key={index} className="bg-input-bg rounded-xl p-3 flex flex-col sm:flex-row items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                            <img src={req.avatar} alt={req.name} className="h-10 w-10 rounded-full" />
                            <div>
                                <p className="font-bold text-sm text-nimasa-dark-text">{req.name}</p>
                                <p className="text-xs text-gray-600">{req.leaveType}</p>
                                <p className="text-xs text-gray-600">{req.dates} ({req.days} days)</p>
                            </div>
                        </div>
                        <button className="bg-nimasa-blue text-white font-bold py-1.5 px-4 rounded-full text-sm shadow-md hover:bg-blue-700 transition-colors w-full sm:w-auto">
                            Review Request
                        </button>
                    </div>
                ))}
