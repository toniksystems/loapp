...
const TeamOnLeave = () => {
    return (
        <div className="bg-white rounded-3xl shadow-md p-8 h-full">
            <h2 className="text-3xl font-medium text-nimasa-dark-text mb-6">Team on Leave</h2>
            <div className="space-y-6">
                {teamOnLeave.map((member, index) => (
                    <div key={index} className="flex items-center gap-4">
                        <img src={member.avatar} alt={member.name} className="h-16 w-16 rounded-full" />
                        <div>
                            <p className="font-bold text-lg text-nimasa-dark-text">{member.name}</p>
                            <p className="text-base text-gray-600">{member.leaveType}</p>
                            <p className="text-base text-nimasa-red font-medium">{member.returnDate}</p>
                        </div>
                    </div>
                ))}
            </div>
