...
const TeamOnLeave = () => {
    return (
        <div className="bg-white rounded-2xl shadow-md p-4 h-full">
            <h2 className="text-lg font-medium text-nimasa-dark-text mb-5">Team on Leave</h2>
            <div className="space-y-4">
                {teamOnLeave.map((member, index) => (
                    <div key={index} className="flex items-center gap-3">
                        <img src={member.avatar} alt={member.name} className="h-10 w-10 rounded-full" />
                        <div>
                            <p className="font-bold text-sm text-nimasa-dark-text">{member.name}</p>
                            <p className="text-xs text-gray-600">{member.leaveType}</p>
                            <p className="text-xs text-nimasa-red font-medium">{member.returnDate}</p>
                        </div>
                    </div>
                ))}
            </div>
