...
const TeamOnLeave = () => {
    return (
        <div className="bg-white rounded-2xl shadow-md p-4 h-full">
            <h2 className="text-base font-medium text-nimasa-dark-text mb-4">Team on Leave</h2>
            <div className="space-y-3">
                {teamOnLeave.map((member, index) => (
                    <div key={index} className="flex items-center gap-2.5">
                        <img src={member.avatar} alt={member.name} className="h-9 w-9 rounded-full" />
                        <div>
                            <p className="font-bold text-xs text-nimasa-dark-text">{member.name}</p>
                            <p className="text-[11px] text-gray-600">{member.leaveType}</p>
                            <p className="text-[11px] text-nimasa-red font-medium">{member.returnDate}</p>
                        </div>
                    </div>
                ))}
            </div>
