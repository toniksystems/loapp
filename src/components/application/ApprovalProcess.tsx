...
const NumberCircle = ({ number }: { number: number }) => (
  <div className="flex-shrink-0 h-6 w-6 bg-nimasa-blue rounded-full flex items-center justify-center text-white font-bold text-sm">
    {number}
  </div>
);
...
const ApprovalStep = ({ number, title, description }: { number: number; title: string; description: string }) => (
  <div className="flex items-start gap-3">
    ...
    <div>
      <h4 className="font-bold text-nimasa-dark-text text-sm">{title}</h4>
...
const ApprovalProcess = () => {
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4">
      <h3 className="text-base font-medium text-nimasa-dark-text mb-5 text-center">Approval Process</h3>
